...
- Deletes files nothing in the book reaches, by walking the real reference graph rather than trusting the manifest. The CSS `url()` scanner is deliberately naive rather than a full parser, so a malformed rule can hide the reference in the rule following it and take a file that was genuinely in use with it. An independent check catches that and warns, naming the file, so read the report; `docs/profiles.md` has the detail.
