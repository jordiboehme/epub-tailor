...
/// Unicode whitespace folding, which turns NBSP into a plain space that
/// `idpf_key` then strips. That derives a different key from every other
/// reader and, because a key IS produced, scrambles the font while reporting
/// success.
#[test]
fn a_non_breaking_space_in_the_identifier_does_not_change_the_font_key() {
    const UNIQUE_ID: &str = "urn:uuid:0000\u{00A0}0000-0000-0000-0000-000000000000";
    let original_font: Vec<u8> = (0..300u32).map(|i| (i % 250) as u8).collect();
    // Obfuscated with the identifier exactly as written, NBSP and all.
    let obfuscated_font = idpf_obfuscate(&original_font, UNIQUE_ID);
...
<package xmlns="http://www.idpf.org/2007/opf" version="3.0" unique-identifier="pub-id">
  ...
  <spine>
    ...
    assert_eq!(
        result.book.resources["OEBPS/fonts/embedded.ttf"].data, original_font,
        "the NBSP must reach the SHA-1 intact, or the font is XORed into garbage"
    );
}
...
/// Same as [`OPF_WITH_TWO_SPINE_CHAPTERS`] plus a second non-spine document,
/// for the two-dropped-ancestors case.
const OPF_WITH_TWO_NESTED_STRAYS: &[u8] = br#"<?xml version="1.0" encoding="UTF-8"?>
<package xmlns="http://www.idpf.org/2007/opf" version="3.0" unique-identifier="pub-id">
  <metadata xmlns:dc="http://purl.org/dc/elements/1.1/">
    <dc:title>Nested Stray Nav Target</dc:title>
    <dc:creator>Author</dc:creator>
    <dc:language>en</dc:language>
    <dc:identifier id="pub-id">urn:uuid:33333333-3333-3333-3333-333333333333</dc:identifier>
  </metadata>
  <manifest>
    <item id="nav" href="nav.xhtml" media-type="application/xhtml+xml" properties="nav"/>
    <item id="ch1" href="chapter1.xhtml" media-type="application/xhtml+xml"/>
    <item id="ch2" href="chapter2.xhtml" media-type="application/xhtml+xml"/>
    <item id="stray" href="stray.xhtml" media-type="application/xhtml+xml"/>
    <item id="stray2" href="stray2.xhtml" media-type="application/xhtml+xml"/>
  </manifest>
  <spine>
    <itemref idref="ch1"/>
    <itemref idref="ch2"/>
...
/// Two dangling ancestors in a chain, so the surviving leaf sits three levels
/// deep behind two drops and must be promoted by two, not one.
const NAV_WITH_TWO_NESTED_DROPPED_PARENTS: &[u8] = br#"<?xml version="1.0" encoding="UTF-8"?>
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops">
<head><title>Nav</title></head>
<body>
<nav epub:type="toc">
<ol>
<li><a href="chapter1.xhtml">Chapter 1</a></li>
<li><a href="stray.xhtml">Stray Outer</a>
<ol>
<li><a href="stray2.xhtml">Stray Inner</a>
<ol>
<li><a href="chapter2.xhtml">Deep survivor</a></li>
</ol>
</li>
</ol>
</li>
...
#[test]
fn a_survivor_under_two_dropped_ancestors_is_promoted_by_two_levels() {
    // The 1-level case is covered above, and it cannot catch a shift that
    // stops accumulating: replacing the dropped-ancestor push `shift + 1`
    // with a constant `1` still promotes a single-drop survivor correctly.
    // Only a chain of two drops separates the two, and this is that chain -
    // the survivor is at level 3 behind two dangling ancestors, so it must
    // come back to level 1, a sibling of Chapter 1.
    let epub = build_epub(&[
        ("mimetype", b"application/epub+zip"),
        ("META-INF/container.xml", common::CONTAINER_XML),
        ("OEBPS/content.opf", OPF_WITH_TWO_NESTED_STRAYS),
        ("OEBPS/nav.xhtml", NAV_WITH_TWO_NESTED_DROPPED_PARENTS),
        ("OEBPS/chapter1.xhtml", CHAPTER1_DOC),
        ("OEBPS/chapter2.xhtml", CHAPTER2_DOC),
        ("OEBPS/stray.xhtml", STRAY_NOT_IN_SPINE),
        ("OEBPS/stray2.xhtml", STRAY_NOT_IN_SPINE),
    ]);
    let out = convert(Input::Epub(epub), &ConvertOptions::default()).expect("converts");
    ...
    assert!(
        nav.contains("chapter1.xhtml") && nav.contains("chapter2.xhtml"),
        "both surviving targets must still be linked:\n{nav}"
    );
    assert_eq!(
        nav.matches("<ol>").count(),
        1,
        "promoted by only one level, the survivor stays nested under Chapter 1:\n{nav}"
    );
}
