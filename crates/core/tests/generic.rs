...
#[test]
fn dropping_dead_weight_does_not_warn_about_the_manifest_that_declared_it() {
    // The safety net scans surviving documents for a dropped file's basename.
    // The package document is not one of them: a manifest declares every
    // resource it owns, so it names the dropped file by construction, and
    // scanning it warned on *every* unreferenced drop this pass has ever made
    // - "the reachability graph may be missing an edge" on a graph that was
    // perfectly correct. The writer regenerates the OPF from the model anyway,
    // so those bytes never reach the output to dangle in the first place.
    let out = convert(
        Input::Epub(common::book_with_manifested_orphan()),
        &opts_for(&["generic"]),
    )
    .expect("converts");

    assert!(
        common::entry(&out.epub, "OEBPS/orphan.png").is_none(),
        "the unreferenced image must still be dropped"
    );
    assert!(
        out.report.warnings.is_empty(),
        "dropping a manifested orphan must be silent, but got: {:?}",
        out.report.warnings
    );
}

#[test]
fn the_dangling_basename_net_still_fires_on_a_real_chapter_mention() {
    // The other half: the exclusion above must not blunt the net. A surviving
    // chapter that mentions a dropped file's basename - in bytes the writer
    // KEEPS, through a reference shape `refs_of` does not extract - is exactly
    // the signal it exists for, and must still be reported.
    let out = convert(
        Input::Epub(common::book_with_orphan_named_in_prose()),
        &opts_for(&["generic"]),
    )
    .expect("converts");
    assert!(
        out.report
            .warnings
            .iter()
            .any(|w| w.message.contains("still mentions orphan.png")),
        "a surviving chapter that mentions the dropped file must still warn: {:?}",
        out.report.warnings
    );
}
