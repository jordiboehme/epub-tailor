...
<div
  ...
  onclick={onClick}
  onkeydown={onKey}
  class="group relative flex cursor-default flex-col rounded-xl border bg-white text-left transition-shadow hover:shadow-md dark:bg-ink-900 {selected
    ? 'border-teal-500 ring-2 ring-teal-500/60 dark:border-teal-400 dark:ring-teal-400/50 dark:shadow-glow'
    : unreadable
      ? 'border-rose-300 dark:border-rose-500/40'
      : 'border-ink-200 dark:border-ink-800'}"
>
  <!-- Cover -->
  <div class="relative aspect-[2/3] overflow-hidden rounded-t-xl bg-ink-100 dark:bg-ink-800">
    {#if hasCover}
      <img
    ...
    {:else}
      <div class="flex h-full w-full flex-col items-center justify-center gap-2 p-3 text-center">
        <span class="text-2xl font-semibold tracking-wide text-ink-400 dark:text-ink-500">
          {initials || "?"}
        </span>
        <span class="line-clamp-3 text-[11px] leading-tight text-ink-500 dark:text-ink-400">
          {title}
        </span>
      </div>
    {/if}
    ...
    <div class="absolute left-1.5 top-1.5 flex flex-col items-start gap-1">
      {#if original.kind === "md"}
        <span class="rounded bg-ink-900/70 px-1 py-0.5 text-[10px] font-medium uppercase tracking-wide text-white">
          md
        </span>
      {/if}
      {#if badge}
        <span
          ...
          class="rounded bg-ink-900/70 px-1 py-0.5 text-[10px] font-medium text-white"
        >
          copy · {badge}
        </span>
      {/if}
      {#if edited}
        <span
          ...
          class="inline-flex items-center gap-0.5 rounded bg-teal-700/85 px-1 py-0.5 text-[10px] font-medium text-white"
        >
    ...
    {#if running}
      <div class="absolute inset-0 flex items-center justify-center bg-ink-950/40 backdrop-blur-[1px]">
        <svg class="h-7 w-7 animate-spin text-white dark:text-teal-300 dark:drop-shadow-glow" viewBox="0 0 24 24" fill="none">
      ...
      <div class="absolute bottom-0 left-0 h-0.5 w-full overflow-hidden bg-white/20">
        <div class="h-full w-1/3 animate-[shimmer_1.1s_ease-in-out_infinite] bg-teal-400 dark:shadow-glow-sm"></div>
      </div>
    {:else if queued}
      <div class="absolute right-1.5 top-1.5 rounded bg-ink-900/60 px-1.5 py-0.5 text-[10px] text-white">
        queued
      </div>
    ...
    {#if !anyBusy}
      <div class="absolute right-1.5 top-1.5 hidden gap-1 group-hover:flex">
        <button
          ...
          class="rounded-full bg-ink-900/70 p-1 text-white transition-colors hover:bg-rose-600"
        >
  ...
  <div class="min-w-0 px-2.5 pb-2 pt-2">
    <p class="truncate text-[13px] font-medium text-ink-800 dark:text-ink-100" title={title}>
      {title}
    </p>
    {#if subtitle}
      <p class="truncate text-[11px] text-ink-500 dark:text-ink-400" title={subtitle}>{subtitle}</p>
    {/if}

    <div class="mt-1.5 border-t border-ink-100 pt-1.5 dark:border-ink-800">
      <FileList {book} />
    </div>
  </div>
</div>
